...
|--------|------|-------------|
| Page Tracker | `TRACK_PAGES` | Tracks access bit of specified enclave pages at each step |
| GPR Tracker | `TRACK_GPRS` | Records general purpose register values from the SSA GPRSGX region |
| IRQ Tracker | `TRACK_IRQ` | Records interrupt firing and timing information |
...
Enables a module but only tracks the specified items. Pass `NULL` for modules that do not require items (e.g. `TRACK_IRQ`).
...
sgx_tracer_add_module(&tracer, /*option=*/TRACK_GPRS | TRACK_IRQ);
```
...
## VCD Output
The generated `.vcd` file can be opened with GTKWave:
